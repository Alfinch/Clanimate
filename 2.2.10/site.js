...
ui = (function() {
    ...
    stage = (function(){
        ...
        update = function() {
            ...
            view.center = new Point(sw * 0.5, sh * 0.5);
        };
    ...
    tooltip = (function() {
        ...
        new_tooltip = function(message, element, target) {
            ...
            show_tooltip = function() {
                ...
                tooltipElement.style.left = (leftOffset > 0 ? leftOffset : 0) + "px";
            },
...
controller = (function(){
    ...
    delete_layer = function(layer) {
        var name = data.get_layer_name(layer),
        callback = function(input) {
            if (data.delete_layer(layer)) {
                ui.timeline
                    .get_layer(layer)
                    .remove();
                ui.stage
    ...
    new_layer = function() {
        ...
        callback = function(input) {
            ...
            var f = data.get_target_frame();
                ...
                ui.prompt({
                    message:  "Layer name too long - enter a name less than 32 characters in length",
                    button1: {
                        name: "Okay",
                        callback: function() {
                            new_layer();
...
// Brush
var brush = new Tool();
brush.minDistance = settings.strokeWidth * 0.5;
brush.points = {};

brush.onMouseDown = function(event) {
    
    // Create a circle
    brush.stroke = new Path.Circle(event.point, settings.strokeWidth * 0.5);
    brush.stroke.fillColor = "black";
};

brush.onMouseDrag = function(event) {
    
    // Calculate vector from path to outline
    var outline = event.delta.normalize();
    outline.length *= settings.strokeWidth * 0.5;
    outline.angle += 90;
    
    // Calculate outline points
    var p1, p2, p3, p4;
    p1 = new Point(event.point.x     + outline.x, event.point.y     + outline.y);
    p2 = new Point(event.point.x     - outline.x, event.point.y     - outline.y);
    p3 = new Point(event.lastPoint.x - outline.x, event.lastPoint.y - outline.y);
    p4 = new Point(event.lastPoint.x + outline.x, event.lastPoint.y + outline.y);
    
    // Create 'join' portion of line
    var join = new Path([ p1, p2, p3, p4 ]);
    join.closePath();
    join.remove();
    
    // Create 'node' portion of line
    var node = new Path.Circle(event.point, settings.strokeWidth * 0.5);
    node.remove();
    
    // Incorporate new line section into stroke
    brush.stroke.remove();
    brush.stroke = brush.stroke.unite(join);
    brush.stroke.remove();
    brush.stroke = brush.stroke.unite(node);
    brush.stroke.fillColor = "black";
};

brush.onMouseUp = function(event) {
    
    var frame = project.activeLayer;
    
    // Remove the current brush stroke from the canvas
    brush.stroke.remove();
    
    // Simplify the path
    if(brush.stroke.type == "compound-path") {
        for(var i = 0; i < brush.stroke.children.length; i++) {
            brush.stroke.children[i].simplify();
        }
    } else {
        brush.stroke.simplify();
    }
    
    // If the current frame has paths in it...
    if(frame.children.length > 0) {
        var merged = false;
        
        // For each path in the current frame...
        for(var i = 0; i < frame.children.length; i++) {
        
            var child = frame.children[i],
                c1 = child.fillColor,
                c2 = brush.stroke.fillColor;
            
            child.remove();
            
            // If the colour of the current brush stroke and this path match, unite them
            if(c1 = c2) {
                child = child.unite(brush.stroke);
            ...
            // If they do not match, subtract the new stroke from this path
            } else {
                child = child.unite(brush.stroke);
                frame.addChild(child);
                child.fillColor = "black";
        ...
        if(!merged)
            frame.addChild(brush.stroke);
            
    // If the current frame has no paths in it add the current brush stroke
    } else {
        frame.addChild(brush.stroke);
    }
};
