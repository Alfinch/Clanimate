    ...
    tools = (function(){
            ...
            for (i = 0; i < visibleGroups.length; i += 1) {
                for (j=0; j < visibleGroups[i].children.length; j += 1) {
                    ...
                    if (hitResult != null && hit === false) {
                        visibleGroups[i].children[j].selected = (hitResult != null);
                        this.selected = hitResult.item;
                        hit = true;
                    } else {
                        visibleGroups[i].children[j].selected = false;
                    }
                }
            }
        ...
        tools.manipulate.onMouseDrag = function(event) {
            if (this.selected == null) {
                if (this.selectBoxFill) this.selectBoxFill.remove();
                if (this.selectBoxBorder) this.selectBoxBorder.remove();
                this.selectBoxFill = new Shape.Rectangle(
                ...
                this.selectBoxBorder = new Shape.Rectangle(
                    new Point(event.downPoint.x, event.downPoint.y),
                    new Point(event.point.x, event.point.y)
                );
        ...
        tools.square.onMouseDown = function(event) {
            this.stroke = new Path.Circle(event.point, settings.get("strokeWidth") * 0.5);
            this.stroke.fillColor = settings.get("strokeColor");
            this.firstDrag = true;
        };
        ...
        tools.square.onMouseDrag = function(event) {
            var tl, tr, br, bl, outer, inner, offset, arcOffset,
            ...
            offset = new Point(settings.get("strokeWidth") * 0.5, 0);
            arcOffset = offset.clone();
            ...
            arcOffset.angle += 90;
            outer.arcTo(
                addPoints(bl, arcOffset),
                addPoints(bl, offset)
            );
            outer.closePath();
            
            offset.length = Math.sqrt(
